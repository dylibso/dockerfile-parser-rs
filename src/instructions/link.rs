...
impl LinkInstruction {
    pub(crate) fn from_record(record: Pair) -> Result<LinkInstruction> {
        let span = Span::from_pair(&record);
        let field = record.into_inner().next().unwrap();

        match field.as_rule() {
            Rule::cmd_exec => Ok(LinkInstruction {
                span,
                expr: ShellOrExecExpr::Exec(parse_string_array(field)?),
            }),
            Rule::cmd_shell => Ok(LinkInstruction {
                span,
...
impl<'a> TryFrom<&'a Instruction> for &'a LinkInstruction {
    type Error = Error;

    fn try_from(instruction: &'a Instruction) -> std::result::Result<Self, Self::Error> {
        if let Instruction::Link(c) = instruction {
            Ok(c)
        } else {
            Err(Error::ConversionError {
                from: format!("{:?}", instruction),
                to: "LinkInstruction".into(),
            })
        }
    }
}
