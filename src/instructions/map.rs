...
impl MapInstruction {
    pub(crate) fn from_record(record: Pair) -> Result<MapInstruction> {
        let span = Span::from_pair(&record);
        let field = record.into_inner().next().unwrap();

        match field.as_rule() {
            Rule::cmd_exec => Ok(MapInstruction {
                span,
                expr: ShellOrExecExpr::Exec(parse_string_array(field)?),
            }),
            Rule::cmd_shell => Ok(MapInstruction {
                span,
...
impl<'a> TryFrom<&'a Instruction> for &'a MapInstruction {
    type Error = Error;

    fn try_from(instruction: &'a Instruction) -> std::result::Result<Self, Self::Error> {
        if let Instruction::Map(m) = instruction {
            Ok(m)
        } else {
