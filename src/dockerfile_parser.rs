...
pub enum Instruction {
    ...
    EnvPwdIsHostCwd(EnvPwdInstruction),
    EnvExeIsHostCwd(EnvExeInstruction),
}
...
impl Instruction {
    ...
    pub fn span(&self) -> Span {
        match self {
            ...
            Instruction::Link(instruction) => instruction.span,
            Instruction::Map(instruction) => instruction.span,
            Instruction::Net(instruction) => instruction.span,
            Instruction::Misc(instruction) => instruction.span,
            Instruction::EnvPwdIsHostCwd(instruction) => instruction.span,
            Instruction::EnvExeIsHostCwd(instruction) => instruction.span,
        }
    }
}
...
impl_from_instruction!(LinkInstruction, Instruction::Link);
impl_from_instruction!(MapInstruction, Instruction::Map);
impl_from_instruction!(NetInstruction, Instruction::Net);
impl_from_instruction!(MiscInstruction, Instruction::Misc);
impl_from_instruction!(EnvPwdInstruction, Instruction::EnvPwdIsHostCwd);
impl_from_instruction!(EnvExeInstruction, Instruction::EnvExeIsHostCwd);
...
impl TryFrom<Pair<'_>> for Instruction {
    ...
    fn try_from(record: Pair) -> std::result::Result<Self, Self::Error> {
        let instruction: Instruction = match record.as_rule() {
            ...
            Rule::link => LinkInstruction::from_record(record)?.into(),
            Rule::map => MapInstruction::from_record(record)?.into(),
            Rule::net => NetInstruction::from_record(record)?.into(),
            Rule::env_pwd_is_host_cwd => EnvPwdInstruction::from_record(record)?.into(),
            Rule::env_exe_name_is_host_exe_name => EnvExeInstruction::from_record(record)?.into(),
